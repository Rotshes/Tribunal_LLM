...
| **DoD 1 — a stranger can submit and read the opinions** | `web/src/components/ChargeSheetForm.jsx` (turn 021) writes a typed charge sheet and `convene()` sends it inline; `POST /api/validate` runs the same imported `g1ChargeSheet` synchronously in front of the background function, so a rejection reaches the browser with every violation named. Reading the opinions has been live since turn 007. **Built and tested; NOT YET AT THE PUBLIC ADDRESS** — Netlify builds are paused until the final deploy, so https://subtle-axolotl-3f3681.netlify.app still serves the turn-017 bundle with no form | **PARTIAL until the final deploy** — the only thing between this and DONE is one build, and `docs/PRE-SUBMISSION.md` blocks on it |
