...
| Build the running project | This repository — eleven turns, each with a record | STARTED |
...
| Interface | `web/index.html`, no build step (decision 0008): three judge columns in fixed order, a failed judge shown as a failure, per-role model picker, and an archive of past proceedings | DONE locally — one screen, all states covered |
| **DoD 1 — a stranger can submit and read the opinions** | A browser submitted T-001 and read three opinions, 31.08 (turn 009) | **DONE locally** — OPEN at a public address, since nothing is deployed |
| **DoD 3 — a case is retrievable by someone who did not submit it** | `GET /api/runs` lists every stored deliberation with all three rulings, and `?id=` returns one in full, rendered by the same two functions as a live run (`netlify/functions/runs.js`, `web/index.html` "Past proceedings", turn 011). RLS stays closed with no policies; reads are served by the backend — `docs/decisions/0010`. A judge that failed is rebuilt from `model_calls` so a retrieved incomplete panel shows a failure, not two rulings | **DONE locally** — OPEN at a public address, with the rest of the app |
| Deployed at a public address | — | **OPEN — the last one.** Everything it needs is committed: `netlify.toml`, four functions, a no-build frontend. It needs Roy's Netlify account and three environment variables |
