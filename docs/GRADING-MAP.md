...
| Interface | `web/index.html`, no build step (decision 0008): three judge columns in fixed order, a failed judge shown as a failure, a per-role model picker that labels models observed to fail, an archive of past proceedings, and a poll-and-wait state for a background run | DONE — one screen, all states covered |
| **DoD 1 — a stranger can submit and read the opinions** | Live at https://subtle-axolotl-3f3681.netlify.app — anyone can open it, convene a tribunal and read three opinions, without being told how | **DONE** — at a public address, turn 012 |
