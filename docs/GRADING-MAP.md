...
| **DoD 1 — a stranger can submit and read the opinions** | Live at https://subtle-axolotl-3f3681.netlify.app: anyone can open it, convene the tribunal on T-001 and read three opinions. **But the page cannot submit a charge sheet** — it only ever sends `case_id`, and `web/index.html` contains the string `charge_sheet` zero times. The backend already accepts one (`deliberate.js` takes `body.charge_sheet` and runs it through G1); the interface has no form for it, and `docs/01-spec.md` §4 assumes one when it says G1 fires "on every form submission" | **PARTIAL** — reading the opinions is done at a public address; submitting a charge sheet is not built |
